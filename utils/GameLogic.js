...
// Tile ID counter for generating unique IDs
let tileIdCounter = 1;
...
function moveRowLeft(row) {
  // 1. 移除空值并紧缩到左侧
  let filtered = row.filter(val => val !== null && val !== 0);
  ...
  // 3. 再次移除空值并填充到长度4
  filtered = filtered.filter(val => val !== null && val !== 0);
...
// Update tile positions after move
