...
export default function HomeScreen() {
  ...
  const handleMove = useCallback(async (direction) => {
    ...
    Animated.parallel([
      ...
      const withNew = addRandomTile(mergedBoard);

      // === 2) 原子提交，仍保持 isSliding=true 隐藏真实瓦片 ===
      unstable_batchedUpdates(() => {
        // 只 dispatch 一次最终棋盘，避免中间态露帧
        dispatch({ type: 'SET_BOARD', payload: withNew });
        dispatch({ type: 'UPDATE_SCORE', payload: state.score + result.score });
      });

      // === 3) 双 rAF 缓冲，确保原生端绘制完成后再切视图 ===
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          // 只为"新砖"做弹入
          animateNewTiles(mergedBoard, withNew);

          // 胜负判定用 withNew（避免用 state.board）
          if (checkWin(withNew) && state.gameState === 'playing') {
            dispatch({ type: 'SET_GAME_STATE', payload: 'won' });
            showWinModal();
          } else if (checkGameOver(withNew)) {
            dispatch({ type: 'SET_GAME_STATE', payload: 'lost' });
            endGame(withNew, state.score + result.score, false).then(() => {
              showLoseModal();
            });
          }

          setMoveCount(prev => prev + 1);
          if (state.hapticsOn && Platform.OS !== 'web' && Haptics) {
